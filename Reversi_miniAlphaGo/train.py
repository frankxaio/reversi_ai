...
class TrainPipeline:
    ...
    def __init__(self, init_model=None):
        ...
        self.lr_multiplier = 1  # 基於KL自適應的調整學習率
        self.temp = 1.0
        self.batch_size = CONFIG['batch_size']  # 訓練的batch大小
        self.epochs = CONFIG['epochs']  # 每次更新的train_step數量
        self.kl_targ = CONFIG['kl_targ']  # kl散度控制
        self.check_freq = 100  # 保存模型的頻率
        self.game_batch_num = CONFIG['game_batch_num']  # 訓練更新的次數
        ...
        for i in range(self.epochs):
            ...
            kl = np.mean(np.sum(old_probs * (
                np.log(old_probs + 1e-10) - np.log(new_probs + 1e-10)),
                                axis=1))
            if kl > self.kl_targ * 4:  # 如果KL散度很差，則提前終止
                break
    ...
    def run(self):
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                time.sleep(CONFIG['train_update_interval'])  # 每10分鐘更新一次模型
